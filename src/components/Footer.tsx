    ...
    <footer className="bg-chocolate-900 text-peanut-100 py-16">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid md:grid-cols-4 gap-8">
          ...
          >
            <div className="flex items-center gap-3 mb-6">
              <motion.span 
                ...
                className="text-4xl md:text-5xl font-extrabold text-peanut-200 font-caveat tracking-tight select-none"
              >
                Zerofy
              </motion.span>
            </div>
            <p className={`text-peanut-300 mb-6 text-xl md:text-2xl leading-relaxed text-crispy ${getFontClass()}`}>
              {t('footer.tagline')}
            </p>
          ...
          >
            <h3 className={`text-xl md:text-2xl font-bold mb-6 text-peanut-200 text-crispy-bold ${getFontClass()}`}>
              Contact Info
            </h3>
            <div className="space-y-4">
              {[
                ...
                >
                  ...
                  <p className={`text-peanut-300 text-lg md:text-xl text-crispy ${getFontClass()}`}>
                    {item.content}
                  </p>
                </motion.div>
              ))}
            ...
            <div className="space-y-3">
              {legalLinks.map((link, index) => (
                <motion.a
                  ...
                  transition={{ delay: 0.1 * index }}
                  href={link.href}
                  className={`text-peanut-300 hover:text-peanut-200 transition-colors block text-lg md:text-xl text-crispy ${getFontClass()}`}
                >
                  {t(link.key)}
                </motion.a>
              ))}
        ...
        >
          <div className="flex flex-col md:flex-row justify-center items-center gap-4">
            <p className={`text-peanut-300 text-center md:text-left text-base md:text-lg text-crispy ${getFontClass()}`}>
              {t('footer.copyright')}
            </p>
          </div>
        </motion.div>
